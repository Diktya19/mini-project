...
public class ProductController {
    ...
    public ModelAndView index(){
        ...
        List<ProductEntity> dataProduct = this.service.getAll();
        // kirim data ke view
        view.addObject("listProduct", dataProduct);
        // menghitung jumlah data
        int jumlahData = dataProduct.size();
        // kirim data ke view
    ...
    public ModelAndView delete(@ModelAttribute ProductDto request){
        ProductEntity result = this.service.getByid(request.getId());
        if (result == null){
            return new ModelAndView("redirect:/product");
        }
    ...
    public ModelAndView detail(@PathVariable ("id") Integer id){
        ProductEntity product = this.service.getByid(id);
        if (product == null){
            return new ModelAndView("redirect:/product");
        }
